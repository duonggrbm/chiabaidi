...
/* FIX: luôn cập nhật lại quyền và trạng thái nút Rút/Dừng sau khi bắt đầu ván mới */
function refreshActionButtons() {
  try {
    const selectors = [
      '#drawBtn', '#hitBtn', '#standBtn', '#stopBtn',
      '[data-action="draw"]', '[data-action="hit"]',
      '[data-action="stand"]', '[data-action="stop"]'
    ];
    const buttons = selectors.flatMap(sel => Array.from(document.querySelectorAll(sel)));
    if (!buttons.length) return;

    // Tìm trạng thái người chơi hiện tại từ các tên biến phổ biến trong game.
    const currentPlayer =
    ...
    buttons.forEach(btn => {
      // Không dùng display:none nếu nút đang được phép thao tác.
      if (canAct) {
        btn.hidden = false;
        btn.disabled = false;
        btn.style.display = '';
    ...
    console.warn('refreshActionButtons:', e);
  }
}
...
function handle(m){
  if(m.t==="state"){state=m.state; roomCode=m.state.room||roomCode; showGame(); render(); return}
  if(m.t==="deal"){myHand=m.hand||[];render();return}
  if(m.t==="draw"){myHand.push(m.card); if(special(myHand)==="Xì Dách"||special(myHand)==="Xì Bàng"||val(myHand)>=21||myHand.length>=5){me.done=true;send(conns.get("host"),{t:"action",id:me.id,done:true,summary:summarize(myHand),hand:myHand})} render();return}
  if(m.t==="mustStop"){me.done=true;send(conns.get("host"),{t:"action",id:me.id,done:true,summary:summarize(myHand),hand:myHand});render();return}
...
function startRound(){
  setTimeout(refreshActionButtons, 0);
...
function hostMessage(m){
  ...
  if(m.t==="drawRequest"){
    ...
    send(c,{t:"draw",card});
    if(p.cardCount>=5)send(c,{t:"mustStop"});
    return;
  }
  if(m.t==="action"){
...
function render(){
  if(!state)return;
  $("players").innerHTML=state.players.map(p=>`<div class="player"><b>${escapeHtml(p.name)}</b><br><span class="badge">${p.role}</span><br>${p.done?"✅ Đã dừng":"🎯 Đang chơi"}</div>`).join("");
...
// FIX: ván 2 trở đi vẫn hiện đúng nút hành động.
window.addEventListener('load', () => {
  refreshActionButtons();
  setTimeout(refreshActionButtons, 100);
  setTimeout(refreshActionButtons, 500);
});

// Khi giao diện thay đổi sau khi chia bài hoặc chuyển ván, kiểm tra lại nút.
